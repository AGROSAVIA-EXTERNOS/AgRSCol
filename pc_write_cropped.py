...
import fiona
file_location="/dbfs/mnt/agrosaviatest/tiba/POLYGON.shp"
...
#file_location = "/dbfs/FileStore/tables/Tibaitata_perimetro.shp"
#file_location = "abfss://testtiff@dlsagrosaviaedcns.dfs.core.windows.net/tiba/Tibaitata_perimetro.shp"
file_location="/dbfs/mnt/agrosaviatest/tiba/POLYGON.shp"
with fiona.open(file_location, "r",) as shapefile:
    shapes = [feature["geometry"] for feature in shapefile]
...
asset_href =find_asset_by_band_common_name(selected_item, "red").href

signed_hrefs = pc.sign(asset_href)


with rasterio.open(signed_hrefs) as src:
  ...
  with rasterio.open("/databricks/driver/maked_test.tif", "w", **out_meta) as dest:
      dest.write(out_image)
      dest.close()
      file_location = "abfss://testtiff@dlsagrosaviaedcns.dfs.core.windows.net/tiba/"
      dbutils.fs.cp("file:/databricks/driver/maked_test.tif" , file_location + "masked_test.tif") 
...
with rasterio.open(signed_hrefs) as src:
  aoi_bounds=features.bounds(shapes[0])
...
aoi_bounds=features.bounds(shapes[0])
with rasterio.open(signed_hrefs) as src:
  test=warp.transform_bounds("epsg:4326", src.crs, *aoi_bounds)
  roi_polygon_src_coords = warp.transform_geom({'init': 'epsg:4326'},
                                             src.crs,                                            
                                             shapes[0])
